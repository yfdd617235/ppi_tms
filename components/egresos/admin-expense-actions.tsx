...
import { Paperclip, X, CheckCircle, XCircle, FileCheck } from 'lucide-react'
import { executeExpenseRequest, rejectExpenseRequest, emitirCheque } from '@/app/(dashboard)/superadmin/egresos/actions'
...
export default function AdminExpenseActions({ egreso }: Props) {
  ...
  async function onEmitirCheque() {
    if (!file) {
      setError('Debes adjuntar el scan o foto del cheque.')
      return
    }
    setError(null)
    startTransition(async () => {
      try {
        ...
        const result = await emitirCheque(fd)
        if (result.error) {
  ...
  const uploadField = (label: string, placeholder: string) => (
  ...
  return (
    <>
      <div className="flex gap-2 flex-wrap justify-end">
        {/* Botón Emitir cheque: solo para cheques en estado pendiente */}
        {esCheque && egreso.estado === 'pendiente' && (
          <Button
            size="sm"
            className="bg-cyan-600 hover:bg-cyan-700 h-8 gap-1"
            onClick={() => { resetState(); setShowEmitir(true) }}
          >
        ...
        {/* Botón Ejecutar: cheque → solo desde cheque_emitido; otros → desde pendiente/enviado */}
        {(!esCheque || egreso.estado === 'cheque_emitido') && (
          <Button
            size="sm"
            className="bg-green-600 hover:bg-green-700 h-8 gap-1"
            onClick={() => { resetState(); setShowExecute(true) }}
          >
            <CheckCircle className="w-3.5 h-3.5" />
            {esCheque ? 'Confirmar cobro' : 'Ejecutar'}
          </Button>
        )}

        {/* Botón Rechazar: disponible en pendiente, enviado y cheque_emitido */}
        <Button
          size="sm"
          variant="outline"
          className="h-8 text-destructive border-destructive/20 hover:bg-destructive/5 gap-1"
          onClick={() => { resetState(); setShowReject(true) }}
        >
      ...
      {/* Dialog Emitir Cheque */}
      <Dialog open={showEmitir} onOpenChange={setShowEmitir}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>Emitir Cheque</DialogTitle>
            <DialogDescription>
              Adjunta el scan o foto del cheque emitido. El cliente podrá verlo mientras espera el cobro.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-2">
            {resumenBox}
            {uploadField('Scan del cheque', 'Haz clic para subir el scan o foto del cheque emitido')}
            <div className="space-y-1.5">
              <Label htmlFor="notas-emitir">Notas (opcional)</Label>
              <Textarea
                id="notas-emitir"
                placeholder="Número de cheque, observaciones..."
          ...
          <DialogFooter showCloseButton>
            <Button
              ...
              onClick={onEmitirCheque}
              disabled={isPending || !file}
            >
              <FileCheck className="w-4 h-4 mr-1.5" />
              {isPending ? 'Procesando...' : 'Emitir cheque'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>

      {/* Dialog Ejecutar / Confirmar cobro */}
      <Dialog open={showExecute} onOpenChange={setShowExecute}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            <DialogTitle>{esCheque ? 'Confirmar Cobro de Cheque' : 'Ejecutar Pago'}</DialogTitle>
            <DialogDescription>
              {esCheque
                ? 'Confirma que el cheque fue cobrado en el banco y adjunta el comprobante.'
          ...
          <div className="space-y-4 py-2">
            ...
            {/* Para cheques: mostrar link al cheque emitido */}
            {esCheque && egreso.cheque_url && (
              <div className="flex items-center gap-2 p-2 rounded-md border border-cyan-200 bg-cyan-50 text-xs">
                <FileCheck className="w-3.5 h-3.5 text-cyan-600 shrink-0" />
                <span className="text-cyan-700 flex-1">Cheque emitido adjunto</span>
                <a
            ...
            {uploadField(
              esCheque ? 'Comprobante de cobro en banco' : 'Evidencia de pago',
              esCheque
                ? 'Haz clic para subir el comprobante bancario del cheque cobrado'
                : 'Haz clic para subir el comprobante de transferencia o cheque'
            )}

            <div className="space-y-1.5">
              <Label htmlFor="notas-ejecutar">Notas (opcional)</Label>
              <Textarea
                id="notas-ejecutar"
          ...
          <DialogFooter showCloseButton>
            <Button
              ...
              disabled={isPending || !file}
            >
              {isPending ? 'Procesando...' : esCheque ? 'Confirmar cobro' : 'Confirmar ejecución'}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
