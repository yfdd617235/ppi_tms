...
export default async function EmpresaLedgerPage({
  params,
  searchParams,
}: {
  ...
  const [{ data: company }, { data: ingresos }, { data: egresos }] = await Promise.all([
    supabase.from('companies').select('razon_social').eq('id', id).single(),
    supabase
      .from('income_requests')
      .select('id, verificado_at, valor_real, valor_neto, comision_ppi, impuesto_4x1000, descripcion, accounts(nombre)')
      .eq('company_id', id)
  ...
  type Entry = { id: string; date: string; tipo: 'ingreso' | 'egreso'; descripcion: string; cuenta: string; cargo: number; abono: number; tarifa: number; impuesto: number }
  ...
  const exportUrl = `/api/ledger/export?company_id=${id}&from=${dateFrom}&to=${dateTo}`
    ...
    <div className="space-y-6">
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Historial cronológico con saldo acumulado</p>
        </div>
