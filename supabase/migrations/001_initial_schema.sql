-- ============================================================
-- PPI TMS — Esquema inicial de base de datos
-- ============================================================
...
-- Cuentas de tesorería por empresa
...
-- Actualizar saldo al verificar un ingreso
...
BEGIN
  IF NEW.estado = 'verificado' AND OLD.estado != 'verificado' THEN
    ...
    SET
      saldo_disponible = saldo_disponible + NEW.valor_real,
  ...
  IF OLD.estado = 'verificado' AND NEW.estado = 'rechazado' THEN
    UPDATE public.accounts
    SET
      saldo_disponible = saldo_disponible - OLD.valor_real,
...
-- Restar saldo al ejecutar un egreso
...
BEGIN
  IF NEW.estado = 'ejecutado' AND OLD.estado != 'ejecutado' THEN
    UPDATE public.accounts
    SET
      saldo_disponible = saldo_disponible - NEW.valor,
...
-- ============================================================
-- Crear bucket: payment-proofs   (soportes de ingresos)
-- Crear bucket: payment-evidence (evidencias de egresos)
-- Configurar políticas de acceso por company_id en ambos buckets
